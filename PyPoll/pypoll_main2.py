...
with open(electioncsvpath) as electioncsv:
    ...
    for row in csvreader:
        ...
        if row[2] not in candidates.keys(): 
            ...
            candidates[row[2]] += 1 # Adds a count to the vote value every time a candidates name is found

# Sort dictionary values/vote counts in descending order
sorted_candidate = {k: v for k, v in sorted(candidates.items(), 
key=lambda item: item[1], reverse=True)}

# Function to covert percentage of candidate votes
